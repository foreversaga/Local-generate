...
export function LongScriptLibraryManager() {
    ...
    function addShot() {
        setDraft((current) => current ? { ...current, shots: [...current.shots, emptyShot(current.shots.length)] } : current);
    }

    function removeShot(index: number) {
        if (!draft || draft.shots.length <= 2 || !window.confirm(`確定刪除分鏡 ${index + 1}？`)) return;
        setDraft({ ...draft, shots: draft.shots.filter((_, shotIndex) => shotIndex !== index) });
    }

    async function save(event: FormEvent<HTMLFormElement>) {
        event.preventDefault();
        if (!draft || busy || !draft.name.trim()) return;
        if (draft.shots.length < 2 || draft.shots.some((shot) => !shot.prompt.trim() || !shot.description.trim() || !Number.isFinite(Number(shot.duration)))) {
    ...
    return <section className={styles.manager} aria-labelledby="long-script-library-heading">
        <div className={styles.header}>
            <div><span>長影片專用</span><h2 id="long-script-library-heading">長影片劇本庫</h2><p>管理由多個分鏡組成的長影片劇本；不會出現在 Single 劇本庫。</p></div>
        ...
        <div className={styles.content}>
            <div className={styles.list} aria-label="長影片劇本清單"><div className={styles.listMeta}>{loading ? "載入中…" : `${visibleScripts.length} 個長影片劇本`}</div>{visibleScripts.map((script) => <article key={script.id} className={`${styles.card} ${draft?.id === script.id ? styles.selected : ""}`}><button type="button" className={styles.cardMain} onClick={() => edit(script)} aria-label={`編輯長影片劇本 ${script.name}`}><strong>{script.name}</strong><span>{script.shots.length} 個分鏡 · {totalDuration(script.shots).toFixed(1)} 秒</span><small>{script.shots[0]?.description || "尚未填寫分鏡描述"}</small></button><button type="button" className={styles.deleteButton} onClick={() => setPendingDelete(script)} aria-label={`刪除長影片劇本 ${script.name}`}>刪除</button></article>)}{!loading && !visibleScripts.length && <p className={styles.empty}>{query.trim() ? "找不到符合的長影片劇本。" : "尚未建立長影片劇本。"}</p>}</div>
            <div className={styles.editorColumn}>{draft ? <form className={styles.editor} onSubmit={(event) => void save(event)}><div className={styles.editorHeading}><div><span>{draft.id ? "編輯長影片劇本" : "新增長影片劇本"}</span><strong>{draft.name || "未命名長影片劇本"}</strong></div><button type="button" onClick={() => setDraft(null)} disabled={busy}>關閉</button></div><label><span>劇本名稱</span><input ref={nameRef} value={draft.name} maxLength={80} onChange={(event) => setDraft({ ...draft, name: event.target.value })} required /><small>{draft.name.length} / 80</small></label><div className={styles.shotToolbar}><strong>{draft.shots.length} 個分鏡 · {totalDuration(draft.shots).toFixed(1)} 秒</strong><button type="button" onClick={addShot} disabled={busy}>新增分鏡</button></div>{draft.shots.map((shot, index) => <article className={styles.shotCard} key={shot.id || index}><div className={styles.shotHeading}><strong>分鏡 {index + 1}</strong><button type="button" onClick={() => removeShot(index)} disabled={busy || draft.shots.length <= 2}>刪除</button></div><div className={styles.shotMeta}><label><span>秒數</span><input type="number" min={0.5} max={60} step={0.5} value={shot.duration} onChange={(event) => updateShot(index, { duration: event.target.value === "" ? NaN : Number(event.target.value) })} required /></label><span>分鏡時間會依順序累加</span></div><label><span>分鏡描述</span><textarea value={shot.description} onChange={(event) => updateShot(index, { description: event.target.value })} required placeholder="場景、動作、構圖與鏡頭…" /></label><label><span>提示詞</span><textarea value={shot.prompt} onChange={(event) => updateShot(index, { prompt: event.target.value })} required placeholder="送給 H3 的此段提示詞…" /></label></article>)}<button type="submit" className={styles.saveButton} disabled={busy || !draft.name.trim()}>{busy ? "儲存中…" : draft.id ? "儲存變更" : "新增長影片劇本"}</button></form> : <div className={styles.editorEmpty}><strong>選擇長影片劇本開始編輯</strong><p>也可以新增空白長影片劇本，再逐格填寫秒數、提示詞與分鏡描述。</p><button type="button" onClick={startNew}>新增長影片劇本</button></div>}</div>
        </div>
