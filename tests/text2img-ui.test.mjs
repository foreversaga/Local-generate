...
test("exposes the FLUX text-to-image tool through the existing Studio route and API", async () => {
  ...
  assert.match(workspace, /juggernaut-xl-v9/);
  assert.match(workspace, /name="image-model"/);
  assert.match(workspace, /name="text-encoder"/);
  assert.match(workspace, /name="adult-mode"/);
  assert.match(workspace, /adultMode: completedJob\.adultMode/);
  assert.match(workspace, /generateText2ImgPrompt\(description\.trim\(\), \{ adultMode, unloadPromptModel \}\)/);
  ...
  assert.match(workspace, /text2img\.encoder\.uncensored\.warning/);
  ...
  assert.match(client, /adultMode: boolean/);
  assert.match(client, /unloadPromptModel = false/);
  assert.match(client, /JSON\.stringify\(\{ description, adultMode, unloadPromptModel \}\)/);
  ...
  assert.match(dictionaries, /"text2img\.encoder\.uncensored\.name"/);
  assert.match(dictionaries, /"text2img\.model\.juggernaut\.name"/);
  assert.match(dictionaries, /"text2img\.adultMode\.on\.name"/);
