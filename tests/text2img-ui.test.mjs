...
test("exposes only FLUX.2 Klein 9B through the existing Studio text-to-image route and API", async () => {
  ...
  assert.match(workspace, /Promise\.all\(recipes\.map\(async \(recipe, index\)/);
  ...
  assert.match(workspace, /id="text2img-prompt-draft"/);
  assert.match(workspace, /value=\{prompt\}[\s\S]*onChange=\{\(event\) => setPrompt\(event\.target\.value\)\}/);
  assert.match(workspace, /prompt: submittedPrompt/);
  assert.match(workspace, /text2img\.generate\.promptOnly/);
  assert.match(workspace, /text2img\.generate\.confirmSingle/);
  assert.match(workspace, /setJobs\(queued\.filter/);
  assert.match(workspace, /setJobs\(completedQueue\)/);
