...
test("uses the same-origin API on the web service", async () => {
  ...
  assert.match(page, /const \[img2imgSteps, setImg2ImgSteps\] = useState<NumberDraft>\(4\)/);
  assert.match(page, /const \[img2imgCfg, setImg2ImgCfg\] = useState<NumberDraft>\(1\)/);
  assert.match(page, /const \[img2imgSeed, setImg2ImgSeed\] = useState<NumberDraft>\(12345\)/);
  ...
  assert.doesNotMatch(page, /setImg2ImgSteps\(Math\.min\(50, Math\.max\(1, Number\(event\.target\.value\) \|\| 1\)\)\)/);
  assert.doesNotMatch(page, /setImg2ImgCfg\(Math\.min\(20, Math\.max\(0, Number\(event\.target\.value\) \|\| 0\)\)\)/);
  assert.doesNotMatch(page, /setImg2ImgSeed\(Math\.min\(2147483647, Math\.max\(0, Number\(event\.target\.value\) \|\| 0\)\)\)/);
  assert.doesNotMatch(page, /setRenderCount\(Math\.min\(20, Math\.max\(1, Number\(event\.target\.value\) \|\| 1\)\)\)/);
  assert.doesNotMatch(page, /width: width === "" \? 736 : width/);
  assert.doesNotMatch(page, /height: height === "" \? 416 : height/);
  ...
  assert.match(page, /batchSeed\(submittedSeed, index\)/);
