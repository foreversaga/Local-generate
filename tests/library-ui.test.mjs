...
test("library picker and preview dialogs preserve keyboard/a11y contracts", async () => {
  ...
  assert.match(picker, /buildAssetNavigation\(scopedAssets, currentPath, scopedFolders, kinds\.length === 1 \? kinds\[0\] : undefined\)/);
  assert.match(picker, /assetSource === "training"/);
  ...
  assert.match(picker, /全選目前資料夾/);
  assert.match(picker, /取消全選/);
  assert.match(picker, /已達選取上限/);
  assert.match(picker, /aria-checked/);
  assert.match(picker, /allowedRoots/);
  assert.match(picker, /素材來源/);
  ...
  assert.match(library, /deleteAssetFolder/);
  assert.match(library, /requestDeleteSelected/);
