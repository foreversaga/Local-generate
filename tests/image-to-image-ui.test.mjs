...
test("Image to Image route mounts the real workspace", async () => {
  ...
  assert.match(workspace, /開始生成/);
  assert.match(workspace, /已儲存至素材庫/);
...
test("Image to Image UI preserves readiness, submit, poll and retry contracts", async () => {
  ...
  assert.match(workspace, /aria-label="隨機產生種子"/);
  ...
  assert.match(workspace, /僅支援 SDXL LoRA/);
  assert.match(workspace, /僅支援 SD1\.5 LoRA/);
  assert.match(workspace, /以 Z-Image 訓練的 LoRA/);
  assert.match(workspace, /characterLoraNameError/);
  assert.match(workspace, /移除 LoRA 名稱即可不使用此設定/);
...
test("Image to Image batch controls, ranges, partial output and history are wired", async () => {
  ...
  assert.match(workspace, /批次模式會為每張圖片自動產生隨機種子/);
  assert.match(workspace, /disabled=\{active \|\| Number\(batchCount\) > 1\}/);
  assert.match(workspace, /第 1 張使用上方設定；第 2 張起於範圍內亂數/);
  assert.match(workspace, /最小值/);
  assert.match(workspace, /最大值/);
  assert.match(workspace, /最小值不可大於最大值/);
  assert.match(workspace, /"partial"/);
...
test("Image to Image history exposes all item parameters and client-validates range steps", async () => {
  ...
  assert.match(filterBlock, /重繪強度 \$\{itemDenoise\} 採樣步數 \$\{itemSteps\} CFG \$\{itemCfg\} 隨機種子 \$\{itemSeed\}/);
  ...
  for (const label of ["denoise", "steps", "seed"]) assert.match(historyBlock, new RegExp(`FIELD_LABELS\\.${label}`));
  assert.match(historyBlock, /<dt>CFG<\/dt>/);
  ...
  assert.match(workspace, /最小值`, bounds\.min, bounds\.max, bounds\.integer, bounds\.step/);
  assert.match(workspace, /最大值`, bounds\.min, bounds\.max, bounds\.integer, bounds\.step/);
  assert.match(workspace, /必須符合步進值 \$\{step\}/);
});
